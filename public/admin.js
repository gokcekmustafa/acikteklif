...
function ensureVehicleConditionLayoutEditorMarkup() {
    ...
    const toolbar = editor.querySelector("#auctionVehicleConditionToolbar");
...
let stateContentSettings = {};
let statePageContent = {};
let stateContentSelectedGroup = null;
const CONTENT_TREE = [
...
function renderContentEditor(selected) {
    const editor = document.getElementById("contentEditor");
    if (!editor)
        return;
    if (!selected) {
      ...
      <form id="pageContentForm">
        ...
        if (form) {
            form.addEventListener("submit", async (e) => {
                e.preventDefault();
                const textarea = document.getElementById("pageEditor_" + pageKey);
                ...
                await safeAction(form, async () => {
                    ...
                    if (res.ok) {
                        ...
                        if (hint) {
                            hint.textContent = res.error || "Hata.";
    ...
    if (form) {
        form.addEventListener("submit", async (e) => {
            ...
            const payload = {};
            for (const field of group.fields) {
                const el = document.getElementById("ce_" + field.key);
                if (el)
                ...
                if (res.ok) {
                    for (const field of group.fields) {
                        if (payload[field.key] !== undefined)
                            stateContentSettings[field.key] = payload[field.key];
                    }
                    if (hint) {
                        hint.textContent = "Kaydedildi.";
                        hint.className = "formHint success";
...
function renderAuctionBidHistoryModal() {
  ...
  `;
    const formatAutoBidLimit = (maxAmount, isActive) => {
