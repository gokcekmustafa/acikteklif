...
function updateHeroStats() {
    ...
    const totalBids = state.listings.reduce((sum, item) => sum + (item.bidCount || item.hasOffer ? 1 : 0), 0);
    ...
    if (vehEl)
        vehEl.textContent = `${formatOptionCount(totalListings || 0)}+`;
    if (bidEl)
        bidEl.textContent = `${formatOptionCount(totalBids || 0)}+`;
    if (usrEl)
        usrEl.textContent = `${formatOptionCount((state.auth.user ? 1 : 0) + Math.floor(totalListings * 0.4))}+`;
}
...
function renderCard(item) {
    ...
    const composerAutoMax = composerActive && String(state.bidComposer.autoMax || "").trim()
