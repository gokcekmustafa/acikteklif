...
function applyFilters(data) {
  return data.filter((item) => {
    ...
    if (state.filters.productGroup && item.productGroup !== state.filters.productGroup) return false;
    ...
    if (state.filters.neighborhood && item.neighborhood !== state.filters.neighborhood) return false;
    ...
    if (state.filters.lotNo && !item.lotNo.toLowerCase().includes(state.filters.lotNo.toLowerCase())) return false;
    return true;
  });
}
