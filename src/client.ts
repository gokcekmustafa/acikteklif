...
function getFilteredCategoryOptions(selectedGroup) {
  if (!selectedGroup) return state.filterOptions.categories || [];
  const byGroup = state.filterOptions.categoriesByGroup || {};
  const groupCategories = byGroup[selectedGroup];
  if (groupCategories && groupCategories.length > 0) return groupCategories;
  return [];
}
...
function applyFilterOptionsPayload(data) {
  ...
  const catByGroup: Record<string, string[]> = {};
...
function readBooleanValue(...values) {
  for (const value of values) {
    ...
    if (["1", "true", "evet", "yes"].includes(folded)) return true;
    if (["0", "false", "hayir", "hayır", "no"].includes(folded)) return false;
  }
  return null;
}
...
function bindEvents() {
  ...
  elements.listingBoxes.addEventListener("click", async (event) => {
    ...
    const favoriteBtn = target.closest(".favoriteToggle") as HTMLElement | null;
    if (favoriteBtn) {
      event.preventDefault();
      await handleFavoriteToggle(favoriteBtn);
      return;
    }
    const bidBtn = target.closest(".bidBtn") as HTMLElement | null;
    if (bidBtn) {
      event.preventDefault();
      await toggleBidComposer(bidBtn);
      return;
    }
    const cancelBtn = target.closest(".bidComposerCancelBtn") as HTMLElement | null;
    if (cancelBtn) {
      event.preventDefault();
      closeBidComposer();
      render();
    ...
    const backdrop = target.closest(".bidComposerBackdrop") as HTMLElement | null;
    if (backdrop) {
      event.preventDefault();
      closeBidComposer();
      render();
  ...
  elements.listingBoxes.addEventListener("change", async (event) => {
    ...
    const autoToggle = target.closest(".bidComposerAutoToggle") as HTMLInputElement | null;
    if (autoToggle) {
      const form = autoToggle.closest(".bidComposerForm") as HTMLFormElement | null;
      const lotNo = normalizeLotNoKey(form?.dataset?.lotNo || state.bidComposer.lotNo || "");
      const amountInput = form?.querySelector(".bidComposerAmount") as HTMLInputElement | null;
      const autoMaxInput = form?.querySelector(".bidComposerAutoMax") as HTMLInputElement | null;
      ...
      if (autoToggle.checked && lotNo) {
        requestAnimationFrame(() => {
          const activeForm = elements.listingBoxes.querySelector(
            `.bidComposerForm[data-lot-no="${lotNo}"]`
          ) as HTMLFormElement | null;
          const nextInput = activeForm?.querySelector(".bidComposerAutoMax") as HTMLInputElement | null;
          nextInput?.focus();
        });
      }
      return;
  ...
  elements.listingBoxes.addEventListener("input", (event) => {
    const target = event.target as HTMLElement;
    const amountInput = target.closest(".bidComposerAmount") as HTMLInputElement | null;
    if (amountInput) {
      state.bidComposer.amount = String(amountInput.value || "").trim();
      return;
    }
    const autoMaxInput = target.closest(".bidComposerAutoMax") as HTMLInputElement | null;
    if (autoMaxInput) {
  ...
  elements.listingBoxes.addEventListener("submit", async (event) => {
    ...
    if (form.closest(".bidComposerForm")) {
      event.preventDefault();
      await handleBidComposerSubmit(form as HTMLFormElement);
    }
  });
...
async function handleRegister() {
  ...
  if (!name || !tcIdentityNo || !phone || !address || !email || !password) {
    setHint(
      ...
      "İsim Soyisim, TC kimlik no, telefon, adres, e-posta ve şifre zorunludur.",
      "error"
    );
    return;
  }

  const accountTypeRadio = document.querySelector('input[name="accountType"]:checked') as HTMLInputElement | null;
  ...
  const agreementCheckbox = document.getElementById("registerAgreement") as HTMLInputElement | null;
  ...
  try {
    ...
    const naText = elements.registerForm.parentElement?.querySelector(".naText") as HTMLElement | null;
    if (naText) naText.style.display = "none";
...
function renderMyBidsModal() {
  ...
  elements.myBidsSummary.textContent = `Toplam ${formatOptionCount(total)} ihale. Kazanılan: ${formatOptionCount(
    winCount
  )}, Güncel lider olunan: ${formatOptionCount(leadingCount)}.`;
  ...
  elements.myBidsRows.innerHTML = rows
    .map((row) => {
      ...
      const currentBid =
        row.currentBid === null || row.currentBid === undefined ? "-" : `${formatMoneyWithoutCents(row.currentBid)} TL`;
      const autoText =
        row.autoBidEnabled && row.autoBidMax
        ...
        <tr>
          ...
          <td class="myBidsActionsCell">
            <div class="rowActionWrap">
              <button class="miniActionBtn" type="button" data-action="set-auto-bid" data-lot-no="${escapeHtml(
                row.lotNo || ""
              )}" data-min-bid="${Number((row.currentBid ?? row.startPrice ?? 0) + Number(row.minIncrement || 0))}" data-auto-max="${Number(
                row.autoBidMax || 0
              )}">
              ...
              ${
                row.autoBidEnabled
                  ? `<button class="miniActionBtn warn" type="button" data-action="disable-auto-bid" data-lot-no="${escapeHtml(
                      row.lotNo || ""
                    )}">Oto Teklif Kapat</button>`
                  : ""
              }
              ${
                row.canRetract
                  ? `<button class="miniActionBtn danger" type="button" data-action="retract-bid" data-lot-no="${escapeHtml(
                      row.lotNo || ""
                    )}">Teklifi Geri Çek</button>`
                  : ""
              }
...
function resolveMyBidStatus(row) {
  if (row?.isWinner) return { label: "Kazandınız", className: "success" };
  if (row?.isLeading) return { label: "Güncel lider sizsiniz", className: "info" };
  if (row?.isEnded) return { label: "Kazanamadınız", className: "danger" };
...
async function refreshFavoritesList() {
  ...
  state.favoriteLotNoSet = new Set(
    items.map((item) => String(item?.lotNo || "").trim().toUpperCase()).filter((value) => !!value)
  );
    ...
    .map((row) => {
      ...
      return `
        <article class="favoriteItem">
          ...
          <button class="miniActionBtn danger" type="button" data-action="remove-favorite" data-lot-no="${escapeHtml(
            row.lotNo || ""
          )}">
            Favoriden Sil
          </button>
...
function getFilterOptionValue(item, filterKey) {
  if (filterKey === "productGroup") return String(item.productGroup || "").trim();
  if (filterKey === "category") return String(item.category || "").trim();
  if (filterKey === "city") return String(item.city || "").trim();
  if (filterKey === "district") return String(item.district || "").trim();
  if (filterKey === "neighborhood") return String(item.neighborhood || "").trim();
  if (filterKey === "vehicleBrand") return String(item.vehicleBrand || "").trim();
  if (filterKey === "vehicleModel") return String(item.vehicleModel || "").trim();
  return "";
}
...
function renderCard(item) {
  ...
  if (isMachineItem) {
    ...
    if (vehicleBits || vehicleDetailLine) {
      vehicleHtml = `<div class="location"><i class="fas fa-car-side"></i> <span>${escapeHtml(
...
async function toggleBidComposer(button) {
  ...
  render();
  requestAnimationFrame(() => {
    const form = elements.listingBoxes.querySelector(`.bidComposerForm[data-lot-no="${lotNo}"]`) as HTMLFormElement | null;
    const amountInput = form?.querySelector(".bidComposerAmount") as HTMLInputElement | null;
    amountInput?.focus();
    amountInput?.select();
  });
...
async function handleAutoBidConfig(button) {
  ...
  const amountInput = prompt(
    `${lotNo} için otomatik teklif üst limitini girin (en az ${formatMoneyWithoutCents(minBid)} TL):`,
    defaultValue
  );
...
async function initTurnstile() {
  ...
  if (!siteKey) return;
  if (!state.turnstile.required) return;
...
async function loadHomepageContent() {
  try {
    ...
    const linkEls = document.querySelectorAll<HTMLAnchorElement>("[data-content-href]");
